function [out1,out2] = trainrp_custom3(varargin)
...
%         deltaWB = ((ggWB>0)*param.delt_inc + (ggWB<0)*param.delt_dec + (ggWB==0)).*deltaWB;
        deltaWB = (ggWB>0)*param.delt_inc.*deltaWB + (ggWB<0).*((deltaWB < 0.4* SA^2).*(param.delt_dec * deltaWB + 0.8 * rand(size(deltaWB))*SA^2) + (deltaWB >= 0.4* SA^2)* param.delt_dec.*deltaWB ) + (ggWB==0).*deltaWB;
