...
yTest = net(xTest);
errorScore = sqrt(sum((yTest - tTest).^2) / length(yTest));
...
%     current = parent; 
    ...
    errorScore_updated = sqrt(sum((yTest - tTest).^2) / length(yTest));

    rocAna = roc_for_ANN([yTest' tTest'],0);
    Az_updated = rocAna.AUC;
    
    if errorScore_updated <= stopError 
        net = net_updated;
        errorScore = errorScore_updated;
        Az = Az_updated;
        break
    end % if
    
    if errorScore_updated > errorScore 
        if (rand < exp( -(errorScore_updated - errorScore)/(k*T) ));
            net = net_updated;
            Az = Az_updated;
            errorScore = errorScore_updated;
            success = success+1;
