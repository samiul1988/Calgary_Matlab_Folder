%%%%%%% this function generates followup database (after 6 months) for analysis %%%%%%%%%%%%%
...
%%% masks remains same, main images will change after 6 months %%%%%
...
database_after6Months.header.T=T; %%%%% Sampling Time
database_after6Months.header.NFFT = NFFT; %%%%%% Number of FFT point 
database_after6Months.header.time_vector = time_vector;
database_after6Months.header.frequency_vector = frequency_vector; %%% for normal FFT
database_after6Months.header.frequency_vector_NFFT = frequency_vector_NFFT; %%% for NFFT-point FFT
database_after6Months.header.L_NFFT=L_NFFT;
...
for i=1:length(control_data_after6Months) %%% for controls after 6 months
    
    %%% load the main image data for control subjects  %%%%%
    root_run_closed = strcat(root_mainImage_control,'\',control_data_after6Months{i});
    root_run_opened = strcat(root_mainImage_control,'\',control_data_after6Months{i});
    ...
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.LGN1=downsample(CLGN1,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.LGN2=downsample(CLGN2,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.LOC1=downsample(CLOC1,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.LOC2=downsample(CLOC2,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.SPA1=downsample(CSPA1,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.SPA2=downsample(CSPA2,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesClosed.Original.V1=downsample(CV1,sampleNumber);
    
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.LGN1=downsample(OLGN1,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.LGN2=downsample(OLGN2,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.LOC1=downsample(OLOC1,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.LOC2=downsample(OLOC2,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.SPA1=downsample(OSPA1,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.SPA2=downsample(OSPA2,sampleNumber);
    database_after6Months.(control_dataBaseline{i}).eyesOpened.Original.V1=downsample(OV1,sampleNumber);
    
    %%% preprocessing of data (EC condition): dc removal and temporal filtering (windowing) %%%%
    ECnoDC=remove_DC(database_after6Months.(control_dataBaseline{i}).eyesClosed.Original);
    ECnoDCwin=do_window(ECnoDC,window_type);
    
    %%%% fft calculation %%%%%
    database_after6Months.(control_dataBaseline{i}).eyesClosed.FFT=do_fft(ECnoDCwin,NFFT);
...
%     database_after6Months.(control_dataBaseline{i}).eyesClosed.TF=do_TransferFunction(database_after6Months.(sprintf('control%d',i)).eyesClosed.FFT,'all',powerOrAmplitude,NFFT);
    ...
    EOnoDC=remove_DC(database_after6Months.(control_dataBaseline{i}).eyesOpened.Original);
    EOnoDCwin=do_window(EOnoDC,window_type);
    %%%% fft calculation %%%%%%
    database_after6Months.(control_dataBaseline{i}).eyesOpened.FFT = do_fft(EOnoDCwin,NFFT);
%     %%%%%  calculate transfer function %%%%%
...
for i=1:length(patient_data_after6Months) %%% for patients after 6 months
    
    %%% load the main image data for patients  %%%%%
    root_run_closed=strcat(root_mainImage_patient,'\',patient_data_after6Months{i},'\6months');
    root_run_opened=strcat(root_mainImage_patient,'\',patient_data_after6Months{i},'\6months');
    %%% load V1 mask 
    ...
    %%%% store the data into database %%%%
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.LGN1=downsample(CLGN1,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.LGN2=downsample(CLGN2,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.LOC1=downsample(CLOC1,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.LOC2=downsample(CLOC2,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.SPA1=downsample(CSPA1,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.SPA2=downsample(CSPA2,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original.V1=downsample(CV1,sampleNumber);
    
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.LGN1=downsample(OLGN1,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.LGN2=downsample(OLGN2,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.LOC1=downsample(OLOC1,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.LOC2=downsample(OLOC2,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.SPA1=downsample(OSPA1,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.SPA2=downsample(OSPA2,sampleNumber);
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original.V1=downsample(OV1,sampleNumber);
    
    %%% preprocessing of data (EC condition): dc removal and temporal filtering (windowing) %%%%
    ECnoDC=remove_DC(database_after6Months.(patient_dataBaseline{i}).eyesClosed.Original);
    ECnoDCwin=do_window(ECnoDC,window_type);
    
    %%%% fft calculation  %%%%%%%
    database_after6Months.(patient_dataBaseline{i}).eyesClosed.FFT=do_fft(ECnoDCwin,NFFT);
...
%     database_after6Months.(patient_data{i}).eyesClosed.TF=do_TransferFunction(database_after6Months.(sprintf('control%d',i)).eyesClosed.FFT,'all',powerOrAmplitude,NFFT);
    ...
    %%% preprocessing of data (EO condition): dc removal and temporal filtering (windowing) %%%%
    EOnoDC=remove_DC(database_after6Months.(patient_dataBaseline{i}).eyesOpened.Original);
    EOnoDCwin=do_window(EOnoDC,window_type);
    
    %%%% fft calculation 
    database_after6Months.(patient_dataBaseline{i}).eyesOpened.FFT=do_fft(EOnoDCwin,NFFT);
...
save(strcat('database_after6Months_',date,'.mat'), 'database_after6Months')
