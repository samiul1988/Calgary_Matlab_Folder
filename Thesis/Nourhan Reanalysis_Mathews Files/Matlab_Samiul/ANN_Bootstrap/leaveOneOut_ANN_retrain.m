...
fileName = strcat('ANN_LOO_retrain_',num2str(featureNum),'11_HLS',num2str(hiddenLayerSize),generateFileNameForANN(data.header));
...
switch dialog
    case 'yes'
        ...
        if strcmp(saveName,'yes')
            save(strcat(fileName,'.mat'), 'ci','rAUCboot','rocAUC','net','numBoot_out','runTime');
        else
            customName = inputdlg('Enter .mat file name:','Save Option',1,{fileName},'on');
            if  ~isempty(customName)
                save(strcat(customName{1},'.mat'), 'ci','rAUCboot','rocAUC','net','numBoot_out','runTime');           
            end
        end
        break;
    case 'no'
